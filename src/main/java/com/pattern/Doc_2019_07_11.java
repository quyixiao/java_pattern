...
public class Doc_2019_07_11 {
    ...
    // 老子论对人之治
    // 就是中国传统哲学而言，所谓的道，就是天下的出路，诸子百家，概莫能出
    // 哲学并不局限于人类社会，哲学可以投放到各个系统中加以应用，这其中的包括软件系统
    // 一个软件可以看成是一个虚似的世界，设计师本人就是驾驭这个世界的统治者，那么中国古代的圣贤的智慧完全可以应用到软件系统中设计中去
    // 创建模式，是对类的实例化过程的抽象化，一些系统创建对象，需要动态的决定怎样创建对象，创建哪些对象，以及如何组合和表示这些对象，创建
    // 模式描述是怎样的构造和封装这些动态决定的
    // 创建模式分为类的创建模式和对象的创建模式两种
    // 类的创建模式，类的创建模式使用继承关系，把类的创建延迟到了子类 ，从而是封装成也客户端将得到哪些具体的类的信息，并且隐藏了这些类的
    // 实例是如何被创建和放在一起的
    // 对象的创建模式，而对象的创建模式则把对象的创建过程动态的委派给另一个对象，从而是动态的决定客户端将得到哪些具体的的类的实例，以及这些
    // 类的实例是如何被创建和组合在一起的
    // 配将要介绍的创建模式包括以下的几种，简单的工厂模式，工厂方法模式，抽象工厂方法模式，单例模式，多例模式，建造模式，原始模型模式等
    // 此外，与单例模式一同介绍的还有专题javaBean的冷藏和解冻
    //
