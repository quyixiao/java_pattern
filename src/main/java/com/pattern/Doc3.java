...
public class Doc3 {
    ...
    // 代码的变异和使用
    // 由于同样或类似的源代码同时被复制多个软件成分中，当这块代码发生程序错误时，需要修改，程序员需要独立的修改每一块拷贝，由于同样的原因，
    // 多个软件成分需要独立的检测，利用的性能节省的初期投资十分有限
    // 算法的复用
    // 各种算法比如算法得到了大量的研究，现在几乎不会而人在应用程序编程时试图建立自己的的排序算法，通常的做法就是在得到了很好的研究的
    // 各种算法中选择一个，这就是算法的利用
    // 数据结构的复用
    // 与算法的复用是想让对的，在数据结构的利用，如队，栈，队列，等数据结构得到，十分透彻的研究，所有的计算都要详细的描述这些数据结构，这就是
    // 数据结构的利用，这些都是传统的复用的种类，它们各有各的优点，也在各自的缺点。
    // 可维护性与可复用性有两个共同的特性，他们是两个独立又具有联系的特性。
    // 因此，一个重要的概念就是支持可维护性的复用，也就是保持甚至提高系统的可维护性的同时，实现系统的复用，那么怎样才能设计出一个这样的系统呢
    // 以达到提高可维护性的复用的目的，换言之，怎样才能抓到同时奔跑的兔子呢。
    // 面向对象的设计的复用。
    // 在一个像 java 这样的面向对象的语言中，数据的抽象化，继承，封装和多态性几期最重要的语言特性，这些特性使得一个系统可以在更加高的层次上
    // 提供可复用性，数据的抽象化和继承关系，使得概念和定义得到复用，多态使得实现和应用可以复用，
    // 现是抽象化和封装可以保持和促进系统的可维护性，这样一来，利用的焦点不再是集中在函数和算法等具体的细节上，而是集中在最重要含有宏观
    // 商业逻辑的抽象层次上，换言之，复用的焦点发生了倒转。
    // 发生复用的焦点的倒转并不是因为实现细节的复用不再重要，而是因为这些细节上的利用偏偏已经做得更加好，而且这些细节更加的强调复用的焦点
    // 因为它们在提高复用的同时保持和提高维护性的关键。
    // 既然抽象层次是一个应用系统做战略性判断和决定的地方，那么抽象层次是一个相应系统做战略性判断和决定的地方，那么抽象层次就就当较为稳定的
    // 应当是复用的重点，如果抽象层次的结构，所有的抽象层次的复用就较为容易。
    // 在面向对象的设计里，可维护性的复用是以设计原则为模式的基础的，请参见下面的分析
    // 
