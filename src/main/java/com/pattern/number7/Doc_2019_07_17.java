...
public class Doc_2019_07_17 {
    ...
    // 由于每个具体的工厂角色都需要负责
    // 在本书的
    // 简单工厂模式一章中，曾经谈到了简单工厂模式中的，工厂角色可以也抽象角色合并，并以java.utils.DateFormate例合并java.util.DateFormate
    // 而是具体的工厂角色
    // 在抽象工厂角色中，通过工厂方法木工或者原始模式实现，
    // 在本章所有例子中，抽象工厂模式都通过工厂方法模式实现的，每一个工厂角色都配有一个工厂方法，这个方法负责调用产品的构造成不是产品角色而
    // 具体的工厂角色
    // 这就意味着要为每一个具体的工厂配备一个静态的方法，而其返回的类型是该具体的工厂类自己
    // 通过工厂方法模式或者原始模模型的模式实现
    // 在本章的所有的例子中，抽象工厂模式都是通过工厂方法模式实现的，每一个工厂角色都本有一个工厂方法，这个方法负责调用产品的构造子，将产品
    // 角色实例化
    // 抽象工厂模式完全可以使用原始模式而不是工厂方法模式实现，读者可以参阅本书的原始模型，Prototype模式一章，这里不再赘述，原始的模式可以
    //简略类图所示
    // 桥梁模式
    // 抽象工厂模式可以为桥梁模式提供了某一个等级结构的创建功能，抽象模式可以与桥梁模式一同使用，桥梁模式的简略类图如下图所示
    // 在两个等级结构结构之间的是java.awt.Toolkit类，这个抽象类是在不同的操作系统中有不同的具体的子类 ，并为每一个peer构件提供了相应的工厂方法
    // 以创建并且返还一个Peer构件实例，如下图所示，图中所示了java构件Button通过调用Tookit工厂对象创建一个ButtonPeer对象的情况
    // 上面的Button和ButtonPeer的通信图，Button是一个java构件，通过Tookie对象与本地Peer构件通信，在这个结构中，Toolkit就是抽象工厂角色
    // Peer对象根据自己的分类形成继承的等级结构，而根据操作系统划分成不同的产品族
    // 两个等级结构之间通过委派进行通信，形成桥梁模式的结构，详细的讨论请参见本章后面的附录，java AWT 的Peer 架构也抽象桥梁模式一章
    // 吃饭，睡觉，说话，
    // 显然，产品类有两个族，为阴和阳，有两个产品等级结构，为人和兽，这种对于一个产品等级结构的情况是需要抽象工厂模式的关键原因
    // 相图与UML图没有关系
    // 在工厂方法模式中，只有一个产品等级结构，工厂等级结构与产品的等级结构是平行的，因此，不难给出这个模式相图，如右图所示
    // 在简单的工厂模式中，只有一个工厂角色和一个产品等级结构，因些不难给出这个模式的相图，如下图所示
    // 需要管理资源包括软件内部资源，譬如 ，大多数软件都有一个甚至多个属性文件存放系统配置，这样的系统应当由一个对象来管理一个属性文件
    // 需要管理软件内部资源也包括譬如负责记录的网站来访问人数的部件，记录软件系统内部事件，出错信息的部件，或者是对系统表现检查部件等，
    // 读者可能会注意到，在上面的懒汉式单例类实现里对静态工厂方法使用同步化的以处理多线程环境，以处理多线程环境，有些设计师在此类不能被
    // 继承，饿，单例类被加载时仍将自己实例化，单从资源的效率角度来讲，这个比懒汉式的单例类的实例化时，必须处理好在多个线程同时这就意味着
    // 出现线程同时首先引用此类的几率变得较大
    // 饿汉式的单例类可以在java语言中内实现，但是不容易C++内出现实现，因为静态初始化C++没有固定的顺序
    // 实际上，本书认为饿汉式，单例类更符合java语言的特点
    // 登记式单例类
    // 登记式单例类是GOF为了克服汉式单例类懒汉工单例类均不可继承的缺点而设计的，本书把他们图中关系栈表明此类已经将自己实例化，
    // 登记式单例的源代码的如代码清单
    //
